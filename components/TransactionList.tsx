...
const TransactionList: React.FC<TransactionListProps> = ({ transactions, onDelete }) => {
  ...
  const handleExportCSV = () => {
    const headers = [t('csv_date'), t('csv_desc'), t('csv_cat'), t('csv_type'), t('csv_amount')];
    const rows = filteredTransactions.map(tx => [
      tx.date, `"${tx.description.replace(/"/g, '""')}"`,
      tx.category, tx.type === 'expense' ? t('csv_expense') : t('csv_income'), tx.amount.toFixed(2)
    ]);
    ...
    <div className="space-y-5">
      ...
      <div className="flex p-1.5 bg-slate-100 dark:bg-slate-800 rounded-2xl w-full max-w-sm mx-auto shadow-inner">
        <button onClick={() => switchTab('expense')}
          ...
          <i className="fas fa-shopping-bag"></i><span>{t('list_expenses')}</span>
        </button>
        <button onClick={() => switchTab('income')}
          ...
          <i className="fas fa-piggy-bank"></i><span>{t('list_income')}</span>
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-darkCard rounded-3xl p-5 border border-slate-100 dark:border-slate-800 flex items-center justify-between">
        <div>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('list_selected_total')}</p>
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('list_tx_count')}</p>
            ...
            <div key={tx.id} className="p-5 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
              <div className="flex items-center space-x-4">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center space-x-2">
                    <span className="text-[10px] text-slate-400 font-bold">{new Date(tx.date).toLocaleDateString(t('locale'), { day: '2-digit', month: 'short' })}</span>
              ...
              <div className="flex items-center gap-3">
                <div className="text-right">
                  ...
                  {tx.source === 'manual' && <span className="text-[8px] bg-slate-100 dark:bg-slate-800 text-slate-400 px-1 rounded font-bold uppercase">{t('list_manual_badge')}</span>}
                </div>
                <button onClick={() => handleDeleteClick(tx.id)}
                  ...
                  title={confirmDelete === tx.id ? t('list_delete_confirm') : t('list_delete')}>
          ...
          {filteredTransactions.length === 0 && (
            <div className="py-20 text-center">
              ...
              <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">
                {searchQuery ? t('list_no_search') : t('list_no_records')}
              </p>
            </div>
          )}
        </div>

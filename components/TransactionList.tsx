...
const TransactionList: React.FC<TransactionListProps> = ({ transactions, onDelete }) => {
  ...
  const categories = useMemo(() => {
    const cats = Array.from(new Set(filteredByType.map(tx => tx.category))).sort();
    return cats;
  }, [filteredByType]);

  const filteredTransactions = useMemo(() => {
    if (selectedCategory === 'all') return filteredByType;
    return filteredByType.filter(tx => tx.category === selectedCategory);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex p-1.5 bg-slate-100 dark:bg-slate-800 rounded-2xl w-full max-w-sm mx-auto shadow-inner">
        <button
          onClick={() => { setActiveType('expense'); setSelectedCategory('all'); }}
        ...
        <button
          onClick={() => { setActiveType('income'); setSelectedCategory('all'); }}
      ...
      {categories.length > 1 && (
        <div className="flex gap-2 flex-wrap">
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${selectedCategory === 'all' ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-darkCard text-slate-400 border border-slate-100 dark:border-slate-800 hover:border-indigo-300'}`}
          >
            Tümü
          </button>
          {categories.map(cat => (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${selectedCategory === cat ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-darkCard text-slate-400 border border-slate-100 dark:border-slate-800 hover:border-indigo-300'}`}
            >
              {cat}
            </button>
          ))}
